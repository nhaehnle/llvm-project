...
bool GenericDomTreeNodeBase::compare(const GenericDomTreeNodeBase *Other) const {
  ...
  SmallPtrSet<const void *, 4> OtherChildren;
  for (const GenericDomTreeNodeBase *I : *Other) {
    const void *Nd = I->getBlock();
    OtherChildren.insert(Nd);
  }

  for (const GenericDomTreeNodeBase *I : *this) {
    const void *N = I->getBlock();
...
bool GenericDominatorTreeBase::properlyDominatesBlock(const void *A,
                                                      const void *B) const {
  if (A == B)
    return false;
...
bool GenericDominatorTreeBase::dominatesBlock(const void *A,
                                              const void *B) const {
  if (A == B)
    return true;
...
const void *GenericDominatorTreeBase::findNearestCommonDominatorBlock(
  ...
  return Dom ? Dom->getBlock() : nullptr;
}
