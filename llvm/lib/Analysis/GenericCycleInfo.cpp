...
bool GenericCycleInfoBase::validateTree() const {
  ...
  for (const GenericCycleBase *cycle : depth_first(getRoot())) {
    if (!cycle->m_parent) {
      if (cycle != getRoot()) {
        errs() << "CycleInfo: non-root cycle with !parent\n";
        return false;
      }
      if (cycle->m_entries.size() != 1) {
        errs() << "CycleInfo: root cycle with != 1 entry\n";
        return false;
      }
      if (!cycle->m_blocks.empty()) {
        errs() << "CycleInfo: by convention, the root cycle should store no "
                  "blocks\n";
        return false;
      }
      if (cycle->m_depth != 0) {
        errs() << "CycleInfo: root cycle depth != 0\n";
        return false;
      }
    } else {
      if (cycle == getRoot()) {
        errs() << "CycleInfo: root cycle has a parent\n";
        return false;
      }
      if (!is_contained(cycle->m_parent->children(), cycle)) {
        errs() << "CycleInfo: cycle not a child of its parent\n";
        return false;
      }
      ...
      for (CfgBlockRef block : cycle->m_blocks) {
        auto mapIt = m_blockMap.find(block);
        if (mapIt == m_blockMap.end()) {
          errs() << "CycleInfo: block not in m_blockMap\n";
          return false;
        }
        if (!contains(cycle, mapIt->second)) {
          errs() << "CycleInfo: cycle block list contains block that is not "
                    "transitively part of the cycle\n";
          return false;
        }
        if (!blocks.insert(block).second) {
          errs() << "CycleInfo: block list contains duplicates\n";
          return false;
        }
      }
      blocks.clear();

      if (cycle->m_entries.empty()) {
        errs() << "CycleInfo: cycle has no entry\n";
        return false;
      }
      for (CfgBlockRef entry : cycle->m_entries) {
        if (!entries.insert(entry).second) {
          errs() << "CycleInfo: entry list contains duplicates\n";
          return false;
        }
        if (!is_contained(cycle->m_blocks, entry)) {
          errs() << "CycleInfo: cycle entry not contained in block list\n";
          return false;
        }
      }
      entries.clear();
    ...
    for (const GenericCycleBase *child : cycle->children()) {
      if (child->m_depth <= cycle->m_depth) {
        errs() << "CycleInfo: child not at higher depth than parent\n";
        return false;
      }
      if (!childDepth) {
        childDepth = child->m_depth;
      } else {
        if (childDepth != child->m_depth) {
          errs() << "CycleInfo: children at different depths\n";
          return false;
        }
      }
    }
  ...
  for (const auto &entry : m_blockMap) {
    ...
    for (const GenericCycleBase *cycle = entry.second; cycle != getRoot();
      ...
      if (!is_contained(cycle->m_blocks, block)) {
        errs() << "CycleInfo: block not contained in cycle(s) indicated by "
                  "m_blockMap[block]\n";
        return false;
      }
    }
