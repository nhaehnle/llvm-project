...
void GenericCycleInfoBase::extendCycle(
    const CfgInterface &iface, GenericCycleBase *cycleToExtend,
    CfgBlockRef toBlock, SmallVectorImpl<CfgBlockRef> *transferredBlocks) {
  ...
  while (!workList.empty()) {
    ...
    if (cycleToInclude) {
      ...
      *childIt = std::move(cycleToInclude->m_parent->m_children.back());
      ...
      for (CfgBlockRef entry : cycleToInclude->m_entries)
...
bool GenericCycleInfoBase::validateTree() const {
  ...
  auto reportError = [](const char *file, int line, const char *cond) {
    errs() << file << ':' << line << ": GenericCycleInfoBase::validateTree: "
           << cond << '\n';
  };
