...
public:
  ...
  BlockRef getHeartBlock(const GenericCycle<CfgTraits> *cycle) const {
    return CfgTraits::unwrapRef(GenericConvergenceInfoBase::getHeartBlock(cycle));
  }

  ConvergenceIntrinsic *getIntrinsic(ValueRef intrinsic) {
    return GenericConvergenceInfoBase::getIntrinsic(CfgTraits::wrapRef(intrinsic));
  }
...
public:
  ...
  BlockRef operator[](size_t idx) const {
    return CfgTraits::unwrapRef(ConvergenceAdjustedPostOrderBase::operator[](idx));
  }
};
