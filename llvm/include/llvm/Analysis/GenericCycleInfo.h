...
public:
  ...
  CfgBlockRef getHeader() const { return m_entries[0]; }

  /// \brief Return whether \p block is an entry block of the cycle.
  bool isEntry(CfgBlockRef block) const {
    return is_contained(m_entries, block);
  }

  /// \brief Return whether \p block is contained in the cycle.
  bool containsBlock(CfgBlockRef block) const {
  ...
  Printable printEntries(const CfgPrinter &printer) const;
  Printable print(const CfgPrinter &printer) const;
  ...
  //@{
  using const_blockref_iterator = std::vector<CfgBlockRef>::const_iterator;
  ...
  //@{
  using const_entry_iterator = SmallVectorImpl<CfgBlockRef>::const_iterator;
...
template <typename CfgTraitsT> class GenericCycle : public GenericCycleBase {
public:
  using CfgTraits = CfgTraitsT;
  using Self = GenericCycle<CfgTraitsT>;
  using BlockRef = typename CfgTraitsT::BlockRef;

  bool isEntry(BlockRef block) const {
    return GenericCycleBase::isEntry(CfgTraits::wrapRef(block));
  }
  BlockRef getHeader() const {
    return CfgTraits::unwrapRef(GenericCycleBase::getHeader());
  }
  bool containsBlock(BlockRef block) const {
    return GenericCycleBase::containsBlock(CfgTraits::wrapRef(block));
  }
  ...
  Printable printEntries() const {
    return Printable([&](raw_ostream &out) {
      out << GenericCycleBase::printEntries(CfgPrinterImpl<CfgTraits>(
          CfgInterfaceImpl<CfgTraits>(CfgTraits::getBlockParent(getHeader()))));
    });
  }
  Printable print() const {
    return Printable([&](raw_ostream &out) {
      out << GenericCycleBase::print(CfgPrinterImpl<CfgTraits>(
          CfgInterfaceImpl<CfgTraits>(CfgTraits::getBlockParent(getHeader()))));
    });
  }
  ...
  auto entries() const { return CfgTraits::unwrapRange(m_entries); }
  auto blocks() const { return CfgTraits::unwrapRange(m_blocks); }
};
...
protected:
  ...
  DenseMap<CfgBlockRef, GenericCycleBase *> m_blockMap;
...
public:
  ...
  void compute(const CfgInterface &iface, CfgBlockRef entryBlock);

  /// Methods for updating the cycle info.
  //@{
  void splitBlock(CfgBlockRef oldBlock, CfgBlockRef newBlock);
  void extendCycle(const CfgInterface &iface, GenericCycleBase *cycleToExtend,
                   CfgBlockRef toBlock,
                   SmallVectorImpl<CfgBlockRef> *transferredBlocks = nullptr);
  //@}
  ...
  GenericCycleBase *getCycle(CfgBlockRef block);
  const GenericCycleBase *getCycle(CfgBlockRef block) const {
  ...
  findExitBlocks(const CfgInterface &iface, const GenericCycleBase *cycle,
  ...
  void dump(const CfgPrinter &printer) const { print(printer, dbgs()); }
  //@}
...
public:
  ...
  void compute(BlockRef entryBlock) {
    GenericCycleInfoBase::compute(
        CfgInterfaceImpl<CfgTraits>(CfgTraits::getBlockParent(entryBlock)),
        CfgTraits::wrapRef(entryBlock));
  }

  void splitBlock(BlockRef oldBlock, BlockRef newBlock) {
    GenericCycleInfoBase::splitBlock(CfgTraits::wrapRef(oldBlock),
                                     CfgTraits::wrapRef(newBlock));
  }

  void extendCycle(Cycle *cycleToExtend, BlockRef toBlock) {
    GenericCycleInfoBase::extendCycle(
        CfgInterfaceImpl<CfgTraits>(CfgTraits::getBlockParent(toBlock)),
        cycleToExtend, CfgTraits::wrapRef(toBlock));
  }
  ...
  Cycle *getCycle(BlockRef block) {
    return static_cast<Cycle *>(
        GenericCycleInfoBase::getCycle(CfgTraits::wrapRef(block)));
  }
  ...
  auto findExitBlocks(const Cycle *cycle,
                      SmallVectorImpl<CfgBlockRef> &tmpStorage) const {
    return CfgTraits::unwrapRange(GenericCycleInfoBase::findExitBlocks(
        CfgInterfaceImpl<CfgTraits>(
            CfgTraits::getBlockParent(cycle->getHeader())),
        cycle, tmpStorage));
  }

  void print(raw_ostream &out) const {
    GenericCycleInfoBase::print(
        CfgPrinterImpl<CfgTraits>(CfgInterfaceImpl<CfgTraits>(
            CfgTraits::getBlockParent(getRoot()->getHeader()))),
        out);
  }
...
struct GraphTraits<GenericCycleBase *>
    : GenericCycleGraphTraits<GenericCycleBase *,
                              GenericCycleBase::const_child_iterator> {};
template <typename CfgTraitsT>
struct GraphTraits<const GenericCycle<CfgTraitsT> *>
    : GenericCycleGraphTraits<
          const GenericCycle<CfgTraitsT> *,
          typename GenericCycle<CfgTraitsT>::const_child_iterator> {};
template <typename CfgTraitsT>
struct GraphTraits<GenericCycle<CfgTraitsT> *>
    : GenericCycleGraphTraits<
          GenericCycle<CfgTraitsT> *,
          typename GenericCycle<CfgTraitsT>::const_child_iterator> {};
