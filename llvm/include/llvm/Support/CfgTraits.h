...
///
/// Use CfgTraits::{toGeneric, fromGeneric} to wrap and unwrap concrete object
/// references.
///
...
public:
  ...
  //   static CfgBlockRef toGeneric(BlockRef);
  //   static CfgValueRef toGeneric(ValueRef);
  //   static BlockRef fromGeneric(CfgBlockRef);
  //   static ValueRef fromGeneric(CfgValueRef);
};
...
///    implement toGeneric / fromGeneric
///  - Derive from CfgTraits using CRTP and implement / override additional
...
public:
  ...
  static CfgParentRef toGeneric(ParentType *parent) {
    return CfgParentRef{parent};
  }

  static ParentType *fromGeneric(CfgParentRef parent) {
    return static_cast<ParentType *>(parent.get());
  }

  using BaseTraits::toGeneric;
  using BaseTraits::fromGeneric;
  ...
  using unwrapping_iterator_base = iterator_adaptor_base<
      ...
      // value_type
      decltype(BaseTraits::fromGeneric(*std::declval<BaseIteratorT>())),
  ...
  using wrapping_iterator_base = iterator_adaptor_base<
      ...
      typename std::iterator_traits<BaseIteratorT>::iterator_category,
      // value_type
      decltype(BaseTraits::toGeneric(*std::declval<BaseIteratorT>())),
      typename std::iterator_traits<BaseIteratorT>::difference_type,
      // pointer (not really usable, but we need to put something here)
      decltype(BaseTraits::toGeneric(*std::declval<BaseIteratorT>())) *,
      // reference (not a true reference, because operator* doesn't return one)
  ...
  struct wrapping_iterator : wrapping_iterator_base<BaseIteratorT> {
    ...
    explicit wrapping_iterator(BaseIteratorT it) : Base(it) {}

    auto operator*() const { return BaseTraits::toGeneric(*this->I); }
  };
...
public:
  ...
  void appendBlockDefs(CfgBlockRef block,
    ...
    auto range = CfgTraits::blockdefs(CfgTraits::fromGeneric(block));
    list.insert(list.end(), CfgTraits::wrapIterator(std::begin(range)),
                CfgTraits::wrapIterator(std::end(range)));
  }
  ...
  CfgBlockRef getValueDefBlock(CfgValueRef value) const final {
    return CfgTraits::toGeneric(
        CfgTraits::getValueDefBlock(CfgTraits::fromGeneric(value)));
  }
};
...
public:
  ...
  void printBlockName(raw_ostream &out, CfgBlockRef block) const final {
    CfgTraits::Printer::printBlockName(out, CfgTraits::fromGeneric(block));
  }
  void printValue(raw_ostream &out, CfgValueRef value) const final {
    CfgTraits::Printer::printValue(out, CfgTraits::fromGeneric(value));
  }
};
