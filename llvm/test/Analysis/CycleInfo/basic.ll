; RUN: opt < %s -cycleinfo -analyze | FileCheck %s -check-prefix=CHECK
...
; CHECK:           Cycle at depth 1: headers(loop)
entry:
  br label %loop
...
; CHECK:           Cycle at depth 1: headers(loop) loop_next
entry:
  br label %loop
...
; CHECK:           Cycle at depth 1: headers(outer_header) outer_latch
; CHECK:               Cycle at depth 2: headers(inner)
entry:
...
; CHECK:           Cycle at depth 1: headers(outer_header)
; CHECK:               Cycle at depth 2: headers(inner_header) inner_latch
entry:
  br label %outer_header
...
; CHECK:           Cycle at depth 1: headers(left)
; CHECK:           Cycle at depth 1: headers(right)
entry:
...
; CHECK:           Cycle at depth 1: headers(second)
; CHECK:           Cycle at depth 1: headers(first)
entry:
  br label %first
...
; CHECK:           Cycle at depth 1: headers(outer_header)
; CHECK:               Cycle at depth 2: headers(right)
; CHECK:               Cycle at depth 2: headers(left)
entry:
  br label %outer_header
...
; CHECK:           Cycle at depth 1: headers(right left)
entry:
  br i1 undef, label %left, label %right
...
; CHECK:           Cycle at depth 1: headers(B D C A)
entry:
...
define void @irreducible_header_without_incoming() {
; CHECK-LABEL: CycleInfo for function: irreducible_header_without_incoming
; CHECK:           Cycle at depth 1: headers(C F A) E D B
entry:
...
; CHECK:           Cycle at depth 1: headers(while.cond) lor.rhs if.then69
; CHECK:               Cycle at depth 2: headers(cond.end61 while.body63 cond.true49) while.cond47
entry:
